...
namespace LuciferMem
{
	#region LuciferMem
	public class Mem
	{
		[DllImport("kernel32.dll")]
		private static extern void GetSystemInfo(out Mem.SYSTEM_INFO lpSystemInfo);
		[DllImport("kernel32.dll")]
		public static extern IntPtr OpenProcess(uint dwDesiredAccess, bool bInheritHandle, int dwProcessId);
		[DllImport("kernel32")]
		public static extern bool IsWow64Process(IntPtr hProcess, out bool lpSystemInfo);
		[DllImport("kernel32.dll")]
		private static extern bool VirtualProtectEx(IntPtr hProcess, UIntPtr lpAddress, IntPtr dwSize, Mem.MemoryProtection flNewProtect, out Mem.MemoryProtection lpflOldProtect);
		[DllImport("kernel32.dll")]
		...
		public static extern UIntPtr Native_VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out Mem.MEMORY_BASIC_INFORMATION64 lpBuffer, UIntPtr dwLength);
		[DllImport("kernel32.dll", EntryPoint = "VirtualQueryEx")]
		public static extern UIntPtr Native_VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out Mem.MEMORY_BASIC_INFORMATION32 lpBuffer, UIntPtr dwLength);
		...
		public string LoadCode(string name, string file)
		{
			...
			if (flag)
			{
				uint privateProfileString = Mem.GetPrivateProfileString("codes", name, "", stringBuilder, (uint)stringBuilder.Capacity, file);
			}
			else
			{
        ...
        public byte[] SunIsKind(string code, long length, string file = "")
        {
            byte[] array = new byte[length];
            UIntPtr code2 = this.GetCode(code, file, 8);
            bool flag = !Mem.ReadProcessMemory(this.pHandle, code2, array, (UIntPtr)(checked((ulong)length)), IntPtr.Zero);
            byte[] result;
            if (flag)
            {
                result = null;
        ...
        public async Task<IEnumerable<long>> AoBScan(long start, long end, string search, bool readable, bool writable, bool executable, string file = "")
        {
            return await Task.Run(async () =>
            {
                ...
                for (int i = 0; i < array.Length; i++)
                {
                    ...
                    aobPattern[i] = (byte)(Convert.ToByte(array[i], 16) & mask[i]);
                }
                ...
                start = Math.Max(start, (long)minimumApplicationAddress.ToUInt64());
                end = Math.Min(end, (long)maximumApplicationAddress.ToUInt64());

                Debug.WriteLine($"[DEBUG] memory scan starting... (start:0x{start:X} end:0x{end:X} time:{DateTime.Now:h:mm:ss tt})");
                ...
                // Scan memory regions
                while (this.VirtualQueryEx(this.pHandle, uintPtr, out memory_BASIC_INFORMATION).ToUInt64() != 0UL
                       && uintPtr.ToUInt64() < (ulong)end
                       && uintPtr.ToUInt64() + (ulong)memory_BASIC_INFORMATION.RegionSize > uintPtr.ToUInt64())
                {
                    bool isValidRegion = memory_BASIC_INFORMATION.State == 4096U
                                         && memory_BASIC_INFORMATION.BaseAddress.ToUInt64() < maximumApplicationAddress.ToUInt64()
                                         && (memory_BASIC_INFORMATION.Protect & 256U) == 0U
                                         && (memory_BASIC_INFORMATION.Protect & 1U) == 0U
                                         && (memory_BASIC_INFORMATION.Type == this.MEM_PRIVATE || memory_BASIC_INFORMATION.Type == this.MEM_IMAGE);

                    if (isValidRegion)
                    {
                        // Apply the access flags
                        bool canRead = (memory_BASIC_INFORMATION.Protect & 2U) > 0U && readable;
                        bool canWrite = (memory_BASIC_INFORMATION.Protect & 4U) > 0U || (memory_BASIC_INFORMATION.Protect & 8U) > 0U || (memory_BASIC_INFORMATION.Protect & 64U) > 0U || (memory_BASIC_INFORMATION.Protect & 128U) > 0U && writable;
                        bool canExec = (memory_BASIC_INFORMATION.Protect & 16U) > 0U || (memory_BASIC_INFORMATION.Protect & 32U) > 0U || (memory_BASIC_INFORMATION.Protect & 64U) > 0U || (memory_BASIC_INFORMATION.Protect & 128U) > 0U && executable;

                        if (canRead || canWrite || canExec)
                        {
                            // Add region if it's valid
                            list.Add(new MemoryRegionResult
                            {
                ...
                // Use parallel tasks to process memory regions
                var results = new ConcurrentBag<long>();
                var tasks = list.Select(region => Task.Run(() =>
                {
                    long[] addresses = this.CompareScan(region, aobPattern, mask);
		...
		private unsafe long[] CompareScan(MemoryRegionResult item, byte[] aobPattern, byte[] mask)
		{
			...
			ulong num;
			Mem.ReadProcessMemory(this.pHandle, item.CurrentBaseAddress, intPtr, (UIntPtr)((ulong)item.RegionSize), out num);
		...
		public UIntPtr VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out Mem.MEMORY_BASIC_INFORMATION lpBuffer)
		{
			...
			if (flag)
			{
				Mem.MEMORY_BASIC_INFORMATION64 memory_BASIC_INFORMATION = default(Mem.MEMORY_BASIC_INFORMATION64);
				UIntPtr uintPtr = Mem.Native_VirtualQueryEx(hProcess, lpAddress, out memory_BASIC_INFORMATION, new UIntPtr((uint)Marshal.SizeOf(memory_BASIC_INFORMATION)));
			...
			{
				Mem.MEMORY_BASIC_INFORMATION32 memory_BASIC_INFORMATION2 = default(Mem.MEMORY_BASIC_INFORMATION32);
				UIntPtr uintPtr = Mem.Native_VirtualQueryEx(hProcess, lpAddress, out memory_BASIC_INFORMATION2, new UIntPtr((uint)Marshal.SizeOf(memory_BASIC_INFORMATION2)));
			...
			else
			{
				...
				else
				{
					...
					if (flag6)
					{
						...
						if (flag9)
						{
							Mem.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)this.mainModule.BaseAddress + array4[0])), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
						}
						else
						{
				...
				else
				{
					...
					else
					{
						...
						if (flag7)
						{
							...
							if (flag10)
							{
								Mem.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)((int)this.mainModule.BaseAddress + array4[0]))), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
							}
							else
							{
		...
		public bool WriteMemory(string code, string type, string write, string file = "", Encoding stringEncoding = null)
		{
			...
			return Mem.WriteProcessMemory(this.pHandle, code2, array, (UIntPtr)((ulong)((long)num)), IntPtr.Zero);
		}
		public bool IsAdmin()
		{
			bool result;
					...
					try
					{
						...
						else
						{
							...
							else
							{
								...
								this.GetModules();
								bool flag6;
								this.Is64Bit = (Environment.Is64BitOperatingSystem && Mem.IsWow64Process(this.pHandle, out flag6) && !flag6);
